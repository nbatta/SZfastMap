...
for j in range(Nsamps):
    ...
    for i in range(10):
        ...
        prof_builder = clusters.ProfileBattagliaFast(
            cosmology=Cosmo4enlib, astropars=Astro4enlib, beta_range=beta_range)
        mass_interp = clusters.MdeltaTranslator(Cosmo4enlib)
        data = clusters.websky_pkcs_read(catfile)

        cat = clusters.websky_decode(data, Cosmo4enlib, mass_interp)
        del data
        ...
        yamps = rprofs[:, 0].copy()
        rprofs /= yamps[:, None]
        amps = (yamps * utils.tsz_spectrum(freq) /
                utils.dplanck(freq) * 1e6).astype(dtype)
        poss = np.array([cat.dec, cat.ra]).astype(dtype)
        profiles = [np.array([r, prof]).astype(dtype) for prof in rprofs]
        del rprofs
        prof_ids = np.arange(len(profiles)).astype(np.int32)

        pointsrcs.sim_objects(shape, wcs, poss, amps,
                              profiles, prof_ids=prof_ids, omap=omap, vmin=vmin)
